...
from . import commun_bp
'''
@login_required
@commun_bp.url_value_preprocessor
...
@app.route('/index')
def index():
    return render_template('templates_commun/index.html',
                           title='Home')
    ...
    if form.validate_on_submit():
        if Ldap.connect_simple():
            ...
            if not u:
                ...
                return redirect(next or url_for('commun_bp.accueil'))
    if current_user.get_id() is not None and current_user.is_authenticated:
        return redirect(url_for('index'))
    else:       
        return render_template('templates_commun/login.html',
                           title='Sign In',
                           form=form)

@commun_bp.route('/accueil/')
@login_required
def accueil():
  return render_template('templates_commun/accueil.html',
                            title='Accueil')

@commun_bp.route('/admin/', methods=['GET', 'POST'])
@login_required
def admin():
    import csv
    ...
    if role == 77:
        ...
        if request.method == 'POST' and form.validate():
            ...
            return render_template('templates_commun/admin.html', title='Admin', form=form)
...
@commun_bp.route('/admin/add_user', methods=['GET', 'POST'])
@login_required
def add_user():
    user_id = session.get('user_id', None)
    ...
    if role == 77:
        ...
        if request.method == 'GET':
            return render_template('templates_commun/add_user.html', 
...
@commun_bp.route('/admin/edit_user')
@login_required
def edit_projet():
...
def unauthorized(e):
    ...
    return render_template('templates_commun/errors.html', title="Unauthorized", msg=msg), 401
...
def unauthorized(e):
    ...
    return render_template('templates_commun/errors.html', title="Page not found", msg=msg), 404

@app.errorhandler(500)
def unauthorized(e):
    db.session.rollback()
    msg = 'L\'administrateur est déjà notifié, désolé pour l\'inconvénient causé.'
    return render_template('templates_commun/errors.html', title="Unexpected error", msg=msg), 500